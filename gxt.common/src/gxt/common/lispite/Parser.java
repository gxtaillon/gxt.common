...
public class Parser {
	...
	protected Maybe<Integer> positiveInteger() {
		return consumeString(new Func1<Character, Boolean>() {
			...
			public Maybe<Integer> func(String i) {
				return Maybe.<Integer>Just(Integer.parseInt(i), "parsed a positive integer");
			}
		});
	}
	...
	protected Maybe<Double> float64() {
		return integer().bind(new Func1<Integer, Maybe<Double>>() {
			public Maybe<Double> func(final Integer i) {
				return character('.').bind(new Func1<Character, Maybe<Double>>() {
					public Maybe<Double> func(Character dot) {
						return positiveInteger().bind(new Func1<Integer, Maybe<Double>>() {
							public Maybe<Double> func(Integer f) {
								// not great
								return Maybe.<Double>Just(Double.parseDouble(i + "." + f), "parsed an integer");
							}
						});
					}
				});
	...
	protected Maybe<String> string(final String s) {
		...
		return chars.subList(charIndex, charIndex + len).bind(new Func1<List<Character>, Maybe<String>>() {
			public Maybe<String> func(List<Character> cs) {
				...
				if (p.equals(s)) {
					...
					return Maybe.<String>Nothing("expected string not found");
				}
			}
		});
	}
	...
	protected Maybe<Character> consumeChar(final Func1<Character, Boolean> pred) {
		return chars.get(charIndex).bind(new Func1<Character, Maybe<Character>>() {
			public Maybe<Character> func(Character c) {
				if (pred.func(c)) {
					++charIndex;
					return Maybe.<Character>Just(c, "consumed");
				} else {
					return Maybe.<Character>Nothing("predicate failed");
				}				
			}
		});
	}
