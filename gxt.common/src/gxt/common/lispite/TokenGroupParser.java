...
public class TokenGroupParser extends Parser {
	...
	protected Maybe<TokenGroup> group() {
		...
		return character('(').bind(new Func1<Character, Maybe<TokenGroup>>() {
			public Maybe<TokenGroup> func(Character c) {
				return butParensWs().bind(new Func1<String, Maybe<TokenGroup>>() {
					public Maybe<TokenGroup> func(final String s) {
						...
						return character(')').bind(new Func1<Character, Maybe<TokenGroup>>() {
							public Maybe<TokenGroup> func(Character a) {
								TokenGroup[] g = new TokenGroup[inner.size()];
								inner.toArray(g);
								TokenGroup[] h = new TokenGroup[innerLit.size()];
								innerLit.toArray(h);
								TokenGroup[] i = new TokenGroup[inner.size()+innerLit.size()];
								ArrayExtension.concat(g, h, i);
								if (i.length == 0) {
									return Maybe.<TokenGroup>Just(TokenGroup.Single(s), "got one");
								} else {
									return Maybe.<TokenGroup>Just(TokenGroup.Group(s, i), "got group");
								}
							}
						});
					}
	...
	protected Maybe<TokenGroup> literal() {
		...
		return character('{').bind(new Func1<Character, Maybe<TokenGroup>>() {
			public Maybe<TokenGroup> func(Character c) {
				return butBrackets().bind(new Func1<String, Maybe<TokenGroup>>() {
					public Maybe<TokenGroup> func(final String s) {
						return character('}').bind(new Func1<Character, Maybe<TokenGroup>>() {
							public Maybe<TokenGroup> func(Character a) {
								return Maybe.<TokenGroup>Just(TokenGroup.Single(s), "got literal");
							}
						});
					}
				});
