...
public class ArrayListProxy<Ta> implements Functor<Ta>,
		Monad<Ta, ArrayListProxyBase> {
	ArrayList<Ta> array;
	...
	public Maybe<List<Ta>> subList(int begin, int end) {
		if (begin < 0 || begin >= array.size() || end < 0
				|| end >= array.size() || begin > end) {
			return Maybe.<List<Ta>> Nothing("array indexes out of bounds");
		} else {
			return Maybe.<List<Ta>> Just(array.subList(begin, end),
